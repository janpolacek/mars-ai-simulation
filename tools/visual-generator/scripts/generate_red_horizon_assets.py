...
def save(root: Path, server: str, name: str, graph: dict, output: str, destination: Path) -> Path:
    ...
    (runs / f"{name}.inputs.json").write_text(json.dumps({"graph_sha256": hashlib.sha256(json.dumps(graph, sort_keys=True).encode()).hexdigest()}, indent=2) + "\n")
...
def main() -> None:
    ...
    parser.add_argument("--canonical", type=Path, help="Selected local RH-01 canonical candidate required for mission-scene generation.")
    ...
    if args.stage in ("all", "canonical"):
        for index, variation in enumerate(selected(rover["canonical_variants"]), start=1):
            ...
            save(root, server, f"rh01-canonical-{index:02d}", graph, output, assets / "rh01" / f"canonical-{index:02d}.png")
    if args.stage == "scene":
        ...
        remote_reference = upload(server, canonical_path.read_bytes(), "canonical.png", "mars-ai-stories/red-horizon/rh01/references")
        for index, variation in enumerate(selected(rover["scene_variants"]), start=1):
            graph, output = build(rover["identity"], f'{rover["scene_instruction"]}\n\nVariation: {variation}', rover["scene_style"], [remote_reference], f"mars-ai-stories/red-horizon/rh01/mission-field-check-{index:02d}", seed=910300 + index, scene=True)
            save(root, server, f"rh01-mission-field-check-{index:02d}", graph, output, assets / "rh01" / f"mission-field-check-{index:02d}.png")
