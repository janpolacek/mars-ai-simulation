...
def canonical(config, vehicle):
    ...
    if vehicle.get("assets"):
        parts.append(
            "Use supplied images only for these design details: "
            + "; ".join(a["instruction"] for a in vehicle["assets"])
        )
...
def scene(description):
    return (
        "The reference images show the same vehicle on a studio background. Use them for vehicle identity only. "
