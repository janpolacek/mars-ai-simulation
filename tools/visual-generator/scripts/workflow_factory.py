...
def build(identity, instruction, style, references, prefix, seed=410101, scene=False):
    ...
    b = add(
        ...
        "Scene description" if scene else "Camera / reference instructions",
        value=instruction,
    )
    ...
    for path in references:
        ...
        pixels = add(
            "ImageScaleToTotalPixels", image=pixels, upscale_method="lanczos", megapixels=0.6, resolution_steps=1
        )
    ...
    for key, spec in graph.items():
        ...
        for name, value in spec["inputs"].items():
            ...
            if isinstance(value, list):
                ...
                desc = (
                    definition["input"].get("required", {}).get(name, definition["input"].get("optional", {}).get(name))
                )
        ...
        nodes.append(
            {
                "id": int(key),
                "type": spec["class_type"],
                "title": spec["_meta"]["title"],
                "pos": [(i // 5) * 470, (i % 5) * 340],
                "size": [420, 300],
                "flags": {},
                "order": i,
                ...
                "inputs": inputs,
                "outputs": [
                    {"name": name, "type": kind, "links": []}
                    for name, kind in zip(definition["output_name"], definition["output"])
                ],
