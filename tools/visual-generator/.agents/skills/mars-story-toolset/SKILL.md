...
Read the root `README.md` before changing inputs or running generation.
`story_set.json` is the source of truth for the shared vehicle-story workflow.
`red_horizon_assets.json` is the source of truth for the separate Red Horizon
brand and review-candidate workflow. Mars and its sample vehicles are examples,
