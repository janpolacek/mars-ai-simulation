...
def build(reference, prompt, prefix, seed=410101):
    ...
    positive = add(
        "FluxKontextMultiReferenceLatentMethod", conditioning=positive, reference_latents_method="index_timestep_zero"
    )
    negative = add(
        "FluxKontextMultiReferenceLatentMethod", conditioning=negative, reference_latents_method="index_timestep_zero"
    )
