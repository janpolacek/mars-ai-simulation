...
def run(server: str, graph: dict, timeout: int = 3600, job_path=None, resume=False) -> dict:
    ...
    if resume and job_path and job_path.exists():
        ...
        if prompt_id not in history and not queued:
            raise RuntimeError(
                f"Cannot locate interrupted job {prompt_id}. Check remote output before moving {job_path} aside and retrying."
            )
    else:
        ...
        def execute(name, instruction, references, scene=False):
            ...
            if args.resume and destination.exists() and graph_path.exists() and history_path.exists():
                ...
                if saved_inputs.get("reference_sha256", {}) != hashes:
                    raise RuntimeError(
                        f"{slug}/{name}: reference content changed or lacks provenance; regenerate this output"
                    )
                if previous.get("status", {}).get("status_str") == "success" and previous["status"].get("completed"):
                    ...
                    return (
                        upload_reference(
                            destination.read_bytes(), f"{name}.png", f"mars-ai-stories/stories/{slug}/{folder}"
                        )
                        if not scene
                        else None
                    )
            (directory / f"{name}.inputs.json").write_text(
                json.dumps(
                    {
            ...
            return (
                upload_reference(content, f"{name}.png", f"mars-ai-stories/stories/{slug}/{folder}")
                if not scene
                else None
            )
