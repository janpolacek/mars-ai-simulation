# Asset manifest: 010-one-mars-year

**Card**: `t_6d180087` (Create visual assets: 010-one-mars-year) — stage `IMAGES`, **reuse**.
**Article**: `website/news/010-one-mars-year.mdx` — route `/news/010-one-mars-year/` once published.
**Timeline step**: `010-operations-and-discovery` — page 3 of 4 (this card; pages 1–2 are the released first-sample and stratigraphy records). Locked milestone row `docs/SCENARIO.md:203` (`3 August 2033 | One Mars year of surface operation completed`).

> Supersedes: the writer's short decision record for this slug (written by parent
> `t_cafff882`; part of the draft package). Its decision — `no new imagery: reuse
...
The brief (`.agents/work/briefs/010-operations-and-discovery.md` §Assets and image decision `:39`) fixes `no new imagery: reuse approved rover plate` for this step. The source pack (`.agents/work/sources/010-one-mars-year.md` C11, Asset finding) verified the candidate against the image index (`.agents/work/image-index.md`) and the site's media resolution chain:
...
- The article frontmatter already declares `media: vehicle-references` with the correct `mediaAlt` entry (verified in `website/news/010-one-mars-year.mdx`)
...
|| Article frontmatter | `media: vehicle-references` + one `mediaAlt` entry (verified in `website/news/010-one-mars-year.mdx:10-12`) |
...
|| Subject | RH-01 Pathfinder in its studio baseline configuration — the commemorative visual for one Mars year of surface operation completed |
|| Story step | `010-operations-and-discovery` — page 3 of 4 (3 August 2033: one Mars year of surface operation completed) |
|| Placement target | `media: vehicle-references` — article lead figure |
|| Alt text | As already declared in `website/news/010-one-mars-year.mdx` (studio reference; illustrative artwork, not mission photography) — **must not be modified to show dust/wear/a Mars-year scene** |
...
|| Field | Value | ||-------|-------|-------|
|| Slug | `010-one-mars-year` |
|| Timeline step | `010-operations-and-discovery` — page 3 of 4 (this card); locked milestone row `docs/SCENARIO.md:203` (`3 August 2033 \| One Mars year of surface operation completed`) |
|| Proposed `simulatedDate` | `2033-08-03` (proposal only — reviewer `t_b3e19a8e` verifies from the locked row) |
|| Brief | `.agents/work/briefs/010-operations-and-discovery.md` §Assets and image decision (`:39` — no new imagery: reuse approved rover plate) |
|| Source pack | `.agents/work/sources/010-one-mars-year.md` (claims C1–C12; C11 = Asset finding) |
|| Canonical source | `docs/SCENARIO.md:203`; source pack C11; image-index `.agents/work/image-index.md`; `website/src/lib/assets.ts:13`; `website/src/lib/media.ts:64` |
|| Draft | `website/news/010-one-mars-year.mdx` — `publication: draft`, `simulatedDate` unset, **`media: vehicle-references` already declared** (verified) |
...
|| Card | `t_6d180087` — `mars-ai-simulator-visuals`, run 404, 2026-09-19 |
|| Status | Manifest complete; reviewer records the release decision and confirms the `010-operations-and-discovery` page-3 mapping |
|| Downstream | Reviewer `t_b3e19a8e` (owns the release decision; confirms page-3 mapping, `simulatedDate: 2033-08-03` from `docs/SCENARIO.md:203`, and the `vehicle-references` reuse); Dev `t_aafb44aa` (commit-and-push message `t_aafb44aa: publish 010 one-Mars-year update`) |
...
||| Check | Method | Result | ||-------|--------|--------|--------|
||| Article media declaration | `grep -n 'media:' website/news/010-one-mars-year.mdx` | **`media: vehicle-references` present** (line 10), with one `mediaAlt` entry (lines 11-12) |
...
||| Card graph | `kanban_show` child `t_b3e19a8e` | Reviewer gate waits on this card; parent draft done |
||| Repository hygiene | `git status --short` | this card rewrites `.agents/work/assets/010-one-mars-year/assets.md` only; nothing committed |

## Public / canon impact and confidentiality check

- **Canon impact:** none. This card creates no image, authorizes no new canon, and adds no media placement beyond what the article already declares. The `vehicle-references` key was already released for the wiki leaf (step 003); this card confirms its further public placement on the anniversary article. The page's public facts are the locked milestone row (`docs/SCENARIO.md:203`) plus released step-010-first-sample and step-010-stratigraphy context.
- **Confidentiality:** no private timeline path, no real facility/operator name, no unreleased milestone value, no trajectory parameter, and no later-step fact appears in any asset string — there are no generated assets, captions, or alt texts to leak. The plate stays as the released studio reference; the alt text must not claim Mars-year wear.
- **Review outstanding:** `mars-ai-simulator-reviewer` (`t_b3e19a8e`) must confirm the `010-operations-and-discovery` page-3 mapping, the `2033-08-03` milestone wording from `docs/SCENARIO.md:203`, and the `vehicle-references` reuse.
- **Next owner/action:** reviewer (`t_b3e19a8e`); no dev/build work is required for any media — the article already declares `media: vehicle-references` and the key resolves mechanically. If the reviewer confirms, the dev card commits with no media-related changes (the wiring is already in place).

## Candidate record

||| Field | Value | ||-------|-------|-------|
||| Candidate count | **0** (none) |
||| Proposed stable filename | — (none; reuse of `docs/vehicles/pathfinder/canonical.png` via the existing `vehicle-references` key) |
||| Subject / story step | RH-01 Pathfinder studio baseline — commemorative visual for one Mars year of surface operation completed (3 August 2033) |
||| Caption | none (the `vehicle-references` key renders no per-plate caption) |
||| Alt text | As already declared in the article frontmatter — studio reference, illustrative artwork, not mission photography; must NOT be modified to show Mars-year wear |
||| Private provenance | `imagegen_request_id`: N/A — no generation performed |
||| Rights / permission | Released by editorial gate `t_1e84c815` (step 003-vehicle-design); already placed on the RH-01 Pathfinder wiki leaf; further public placement confirmed by this card |
||| Reuse plate (documented) | `docs/vehicles/pathfinder/canonical.png` — `released`, SHA `8dbb33d5…`, 1448×1086 (4:3), 1,889,350 B |
||| Barred plate (documented) | `docs/vehicles/pathfinder/contact-arm-scene.png` — **withheld** (`scripts/guards.mjs`); the dossier's other four studio renders are approved references nothing places |

The absence of generated candidates is the deliverable: the article already carries the `vehicle-references` media key, which resolves to the approved `canonical.png` plate through the site's existing resolution chain. No generation, no new key, no site-engineering change.

## Handoff

- **Next recommended owner:** `mars-ai-simulator-reviewer` (`t_b3e19a8e`) — confirms canon, verifies `simulatedDate: 2033-08-03` from `docs/SCENARIO.md:203`, and records the release decision for the `vehicle-references` reuse on this page
- **Then:** `mars-ai-simulator-dev` (`t_aafb44aa`) — the media wiring is already in place; the dev card commits with no media-related source changes
