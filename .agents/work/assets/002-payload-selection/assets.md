...
| Sensor-focused editorial illustration (new) | Same band, instead of the identity mark | Not written — alt text is written from a produced candidate, never before one exists | Not decided | Local ComfyUI (Flux.2 Klein through `image_generate`, `provider: comfyui`) — **not run** | To be recorded per candidate before any use; must never claim documentary mission imagery | **Blocked, not merely undecided** — it needs a **new media key in the site** (engineering card: `src/lib/media.ts` key + requirements, `src/features/news/media.ts` resolver, `src/lib/assets.ts` import) *and* an explicit human go-ahead. `newsMediaKeys` currently holds only `programme-identity` and `asteria-plates`, so the illustration cannot be placed today even if it existed. Pattern to copy: `t_3922071d`, which added `asteria-plates` that way. Brief below |
