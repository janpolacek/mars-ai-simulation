...
**Nothing on the page exceeds the released record.** The page is strictly _less_ specific than the
...
- **Copy and metadata surfaces:** 36 strings — the 22 About copy strings, the 7 `/404` strings, and
  ...
  machinery). **1 hit**, and it is benign: the phrase naming the three _unsettled_ technical
...
- **Whole-file scans of the three working papers** (70 / 56 / 87 raw pattern matches): every match
  ...
  _filename_ of a different working paper cited by path; and category words used to _describe_ what
...
| Drift                                                                                                 | None found. The About sentence _echoes_ one approved clause and extends it with "fictional national teams, not real institutions", which is `docs/SCENARIO.md`'s own wording, not a new disclosure claim                           |
...
1. **Universal process wording vs two pre-pipeline surfaces.** §"How a page is made" states the
   ...
   changing it). Read as a statement about _the experiment_ (the sentence's own antecedent) or about
   _the documented process_ (which is what `docs/INSTRUCTIONS.md` and the workflow skill define),
