...
- The About page is not the mission-facts page. The facts panel answers "when?" with the released
  answer to *that* question — the exact dates are not public — while the published article owns the
...
are anchors *inside* a sentence, while the 404's are standalone links under a separate paragraph.
