 ...
 * plates back, one of which (the pad lift-off, `lunch.png`) step 005 released
 * on 2026-09-19 (`.agents/work/reviews/005-launch.md` §10). The withheld set is
 * therefore one directory plus the individually withheld files below
 * (`gatedDirectoryNames` and `gatedFilePathSegments`).
 ...
 * withheld two more fail-closed. Step 005 released the pad lift-off plate
 * (`lunch.png`) on 2026-09-19 (`.agents/work/reviews/005-launch.md` §10), so a
 * single Ariane 64 plate remains withheld here: the in-transit plate, which
 * carries a roundel mark and unapproved burned-in text. It joins the same
 * per-file rule so an import of it fails the build; the released plates are
